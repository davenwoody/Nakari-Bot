...
const jadibot = async (Nakari, text, m) => {
const { sendImage, sendMessage } = Nakari;
...
Nakari.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

Nakari.ws.on('CB:call', async (json) => {
...
Nakari.sendNode({
  tag: 'call',
    attrs: {
      from: '6287883612628@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
let qutsnya = await Nakari.sendContact(callerId, owner)
await Nakari.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
await Nakari.updateBlockStatus(callerId, "block")
}
})
...
m = smsg(Nakari, kay, store)
require('./XeonCheems8.js')(Nakari, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
Nakari.ev.on("creds.update", saveCreds);
Nakari.ev.on("connection.update", async up => {
...
if (connection == "open") {
Nakari.id = Nakari.decodeJid(Nakari.user.id)
Nakari.time = Date.now()
global.conns.push(Nakari)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${Nakari.decodeJid(Nakari.user.id)}*_`)
user = `${Nakari.decodeJid(Nakari.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); Nakari.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Nakari.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Nakarice Logged Out, Please Scan Again And Run.`); Nakari.logout(); }
else if (reason === DisconnectReason.restartRequired) { 
console.log("Restart Required, Restarting..."); start(); }
else if (reason === DisconnectReason.timedOut) { 
console.log("Connection TimedOut, Reconnecting..."); start(); }
else Nakari.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
})
...
for (let i of kon) {
list.push({
displayName: await Nakari.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await Nakari.getName(i + '@s.whatsapp.net')}\n
FN:${await Nakari.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
Nakari.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}
